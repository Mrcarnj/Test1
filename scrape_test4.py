...
#     # Check if the player name already exists in the table
#     query = f"SELECT COUNT(*) FROM test1 WHERE player_name = {player_name}"
#     cursor.execute(query)
#     result = cursor.fetchone()
#     count = result[0]

#     if count > 0:
#         # Update the score if the player name already exists
#         query = f"UPDATE test1 SET score = {score} WHERE player_name = {player_name}"
#     else:
#         # Insert both the player name and score if the player name does not exist
#         query = f"INSERT INTO test1 (player_name, score) VALUES ({player_name}, {score})"
